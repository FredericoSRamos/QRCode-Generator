...
def encode_byte(data):
    try:
        # Try to encode in standard QR Code byte encoding mode
        encoded_data = data.encode('ISO-8859-1')
    except:
        # If it fails, encode in UTF-8, that can't be read by all QR Code readers
...
def encode_data(data, version, encode_type, error_correction_level):
    ...
    required_bits = data_codewords_number[version - 1][error_correction_level] * 8
...
def split_data(data_codewords, version, error_correction_level):
    error_correction_table ={
        'L': {
            ...
            33: {"group1": (34, 115), "group2": (6, 116)},
            34: {"group1": (16, 115), "group2": (28, 116)},
            35: {"group1": (30, 115), "group2": (20, 116)},
            36: {"group1": (22, 115), "group2": (24, 116)},
            37: {"group1": (33, 115), "group2": (15, 116)},
            38: {"group1": (12, 121), "group2": (28, 122)},
            39: {"group1": (11, 121), "group2": (31, 122)},
            40: {"group1": (19, 121), "group2": (26, 122)}
        },
        'M': {
        ...
        'Q': {
            ...
            33: {"group1": (11, 24), "group2": (46, 25)},
            34: {"group1": (59, 24), "group2": (1, 25)},
            35: {"group1": (22, 24), "group2": (41, 25)},
            36: {"group1": (2, 24), "group2": (64, 25)},
            37: {"group1": (24, 24), "group2": (46, 25)},
            38: {"group1": (42, 24), "group2": (32, 25)},
            39: {"group1": (10, 24), "group2": (67, 25)},
            40: {"group1": (20, 24), "group2": (61, 25)}
        },
        'H': {
            ...
            35: {"group1": (44, 15), "group2": (22, 16)},
            36: {"group1": (39, 15), "group2": (25, 16)},
            37: {"group1": (46, 15), "group2": (25, 16)},
            38: {"group1": (49, 15), "group2": (24, 16)},
            39: {"group1": (48, 15), "group2": (28, 16)},
            40: {"group1": (43, 15), "group2": (36, 16)}
        }
    }

    config = error_correction_table[error_correction_level][version]
...
def structure_final_message(encoded_data, version, error_correction_level):
    codewords = [int(encoded_data[i:i+8], 2) for i in range(0, len(encoded_data), 8)]

    groups = split_data(codewords, version, error_correction_level)
    ...
    ecc_count = error_correction_codewords[version - 1][error_correction_level]
...
def get_format_string(error_correction_level, mask):
...
def mask_data(matrix, version, error_correction_level):
    size = version * 4 + 17

    masked_matrices = apply_masks(matrix, size)

    for i in range(len(masked_matrices)):
        format_string = get_format_string(error_correction_level, i)
